          ...
          <div className="col-md-3">
            ...
            {variants.map((variant) => (
              <div key={variant.id} className="mb-4">
                ...
                <div id={`collapse${variant.id}`} className="collapse" aria-labelledby={`collapseToggle${variant.id}`}>
                  {variant.values.map((value) => (
                    <div key={value.value} className="list-group">
                      <label className="list-group-item-action" htmlFor={`variant-${value.value}`}>
                        <input
            ...
            {/* button apply filter */}
            <div className="card mb-4">
              <button className="btn btn-outline-primary p-1" onClick={handleApplyFilter} disabled={processing}>
                {processing ? 'Caricamento...' : 'Applica Filtro'}
              </button>
            </div>
            <hr />
            <div className="mb-4">
