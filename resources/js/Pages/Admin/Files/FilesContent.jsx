            ...
            MySwal.fire({
                ...
                if (result.isConfirmed) {
                    formDelete(route("files.destroy", { fileName: fileName }), {
                        onSuccess: () => {
                            toast.success(
                                `File: ${fileName} - cancellato correttamente`
                            );
                        },
                        onError: () => {
                            toast.error(
                                `Errore durante la cancellazione del file: ${fileName}`
                            );
                        },
                    });
                }
    ...
    const handleCreateFile = async (e) => {
        ...
        if (file) {
            ...
            router.post(route("files.store"), formData, {
                onSuccess: () => {
                    toast.success(
                        `File: ${file.name} - inserito correttamente`
                    );
                },
                onError: () => {
                    toast.error(
                        `Errore durante l'inserimento del file: ${file.name}`
                    );
                },
            });
        }
                        ...
                        <div className="card-body">
                            ...
                            <div className="fm-menu">
                                <div className="list-group list-group-flush">
                                    ...
                                    <Link
                                        href={route("files.documents")}
                                        className={`list-group-item py-1 d-flex align-items-center ${
                                            endLink("/files/documents")
                                                ? `item-active`
                                                : ``
                                        }`}
                                    >
                                    ...
                                    <Link
                                        href={route("files.images")}
                                        className={`list-group-item py-1 d-flex align-items-center ${
                                            endLink("/files/images")
                                                ? `item-active`
                                                : ``
                                        }`}
                                    >
