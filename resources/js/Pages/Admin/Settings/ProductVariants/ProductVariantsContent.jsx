...
const ProductVariantsContent = ({ variants, variants_values, flash }) => {
      ...
      MySwal.fire({
        ...
        if (result.isConfirmed) {
          router.delete(route('settings.variants.destroy', variant), {
            onSuccess: () => {
              toast.success(`Variante: ${variantName} eliminata correttamente`);
            },
            onError: () => {
              toast.error(`Errore durante l'eliminazione della variante: ${variantName}`);
            },
          });
        }
      });
  ...
  const handleValueDelete = (e, variant_value) => {
    ...
    router.delete(route('settings.variant-values.destroy', variant_value), {
      onSuccess: () => {
        toast.success(`Valore eliminato correttamente`);
      },
      onError: () => {
        toast.error(`Errore durante l'eliminazione del valore`);
      },
    });
  };
      ...
      <div className="row">
        <div className="col-md-6">
          <div className="d-grid gap-2 d-md-flex justify-content-md-start">
            <Link href={route('settings.variants.create')} className="btn cb-primary mb-3">
              Inserisci una nuova variante
            </Link>
          </div>
                      ...
                      variants.map((variant) => (
                        <tr key={variant.id} className="align-middle">
                          ...
                          <td scope="row" className="col-auto text-center">
                            ...
                            <form
                              onSubmit={(e) => handleVariantDelete(e, variant)}
                              className="d-inline"
        ...
        <div className="col-md-6">
          <div className="d-grid gap-2 d-md-flex justify-content-md-start">
            <Link href={route('settings.variant-values.create')} className="btn cb-primary mb-3">
                      ...
                      variants_values.map((variant_value) => (
                        <tr key={variant_value.id} className="align-middle">
                          ...
                          <td scope="row" className="col-auto text-center">
                            ...
                            <form
                              onSubmit={(e) => handleValueDelete(e, variant_value)}
                              className="d-inline"
                              id={variant_value.id}
                            >
                              <ButtonDelete url={BASE_URL} />
