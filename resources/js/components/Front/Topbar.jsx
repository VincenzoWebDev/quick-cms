          ...
          <div className="collapse navbar-collapse" id="navbarCollapse">
            <ul className="navbar-nav ms-auto mb-2 mb-md-0">
              <li className="nav-item">
                <Link className="nav-link active rolling-text" aria-current="page" href={route('home')}>
                  Home
                </Link>
              </li>
              {pages.map(
                (page) =>
                  page.active == 1 && (
                    <li className="nav-item" key={page.id}>
                      <Link
                        className="nav-link active rolling-text"
                        aria-current="page"
              ...
              {user_auth && ecommerce_status === '1' && (
                <li className="nav-item dropdown">
                  <a className="text-white px-1" role="button" data-bs-toggle="dropdown" aria-current="page">
                    <i className="fa-solid fa-user"></i>
                  </a>
                  <ul className="dropdown-menu dropdown-menu-end bg-white">
                    <li>
                      <Link className="dropdown-item text-dark" href={route('user.profile.index')}>
                        Profilo
                      </Link>
                    </li>
                    <li>
                      <a className="dropdown-item text-dark" href="#" onClick={handleLogout}>
                        Logout
                      </a>
                    </li>
                  </ul>
